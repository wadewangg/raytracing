...
/*    for (int i = 0; i < 3; i++)
        out[i] = a[i] + b[i];
	__m256d AVXa = __mm256_set_pd(a);
	__m256d AVXb = __mm256_set_pd(b);
	__m256d AVXv = __mm256_add_pd(AVXa, AVXb);
	
	_mm256_store_ps(&out[0], AVXv);
*/
	__m256d AVXa = _mm256_set_pd(a[0],a[1],a[2],0.0);
	__m256d AVXb = _mm256_set_pd(b[0],b[1],b[2],0.0);
	__m256d AVXc = _mm256_add_pd(AVXa,AVXb);
	double temp[4];
	_mm256_store_pd(temp,AVXc);
	out[0] = temp[3];
	out[1] = temp[2];
	out[2] = temp[1];
}
